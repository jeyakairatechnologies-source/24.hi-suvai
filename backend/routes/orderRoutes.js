...
router.post('/', async (req, res) => {
  try {
    ...
    // Automatically decrease inventory stock for ordered products
    try {
      ...
      for (const item of items) {
        ...
        if (productDoc) {
          const currentStock = productDoc.stock !== undefined ? Number(productDoc.stock) : 50;
          const updatedStock = Math.max(0, currentStock - qtyToDeduct);
          productDoc.stock = updatedStock;
          productDoc.isAvailable = updatedStock > 0;
          await productDoc.save();
          console.log(`[Inventory Auto-Deduct] Product "${productDoc.name}" stock reduced: ${currentStock} -> ${updatedStock} (Order ${generatedOrderId})`);
        }
      }
    } catch (stockErr) {
      console.warn('[Inventory Auto-Deduct Warning]:', stockErr.message);
    }
...
// @desc    Get all orders (Admin with search, status filter, pagination)
// @access  Public/Admin
